...
public: // constructors
  ...
  Fixed(long double n) : data_(static_cast<base_type>(n * one))  {
    // TODO(eteran): assert in range!
  }
...
public: // conversion to basic types
  ...
  double to_ldouble() const    {
    return static_cast<long double>(data_) / Fixed::one;
  }
...
//  operator float() { return to_float(); }
  operator long double() const { return to_ldouble(); }
